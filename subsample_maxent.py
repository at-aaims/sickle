...
"""
TODO:
- instead of loading all data, load locally on each rank for data based on the timestep
* dataloader outputs filenames or list of snapshots to be analyzed
* task list made using snapshot filename list
* local dataloader used for loading data on each rank
"""
...
# Initialize MPI
comm = MPI.COMM_WORLD
rank = comm.Get_rank()  # The process ID (rank) of the current process
size = comm.Get_size()  # Total number of processes
if rank==0: print(f"Total ranks: {size}")

# Start the timer (record the start time)
...
# ====================
# time_step func
def time_step(timestep):
    """
    TODO:
    - Migrate from using global vars to func arguments
    """
    ...
    if args.noseed:
        ...
        kmeans = KMeans(n_clusters=args.num_clusters, random_state=random_seed)
    kmeans.fit(data)
    print(f'# clusters: {args.num_clusters}, kmeans inertia: {kmeans.inertia_}') # for creating elbow plot
    centroids = kmeans.cluster_centers_
    cluster_labels = kmeans.labels_
    y_pred = kmeans.predict(data)
    elpsdt = time.time() - t0
    print(f'Time elapsed for Kmeans: {int(elpsdt/60)} min {elpsdt%60:.2f} sec')
    ...
    if args.plot:
        ...
        plt.axis('equal')
        plt.savefig(os.path.join(PLTDIR, f'{fileprefix}_adj_matrix_{timestep:04d}.png'), dpi=100)
    ...
    if args.plot:
        plt.clf()
        colors_ = plt.colormaps.get_cmap('tab10').resampled(args.num_clusters)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(PLTDIR, f'{fileprefix}_prob_dists_{timestep:04d}.png'), dpi=100)
    ...
    if args.subsample == "proportional":
        ...
        if args.plot:
            ...
            plt.title('Histogram')
            plt.savefig(os.path.join(PLTDIR, f'{fileprefix}_histogram_{timestep:04d}.png'))
    ...
    if args.knn > 0:
        ...
        plt.savefig(os.path.join(PLTDIR, f'{fileprefix}_knn_{timestep:04d}.png'), dpi=100)
    ...
    if args.plot:
        # spatial plot with samples colored as black points
        plt.clf()
        ...
        if args.dims == 3:
            ...
            if args.dtype == 'npz' or args.dtype == 'sst-binary':
                x_indices, y_indices, z_indices = np.unravel_index(indices2, (x.shape[0], y.shape[0], z.shape[0]))
                ax.scatter(x[x_indices], z[z_indices], y[y_indices], c='k', s=2, alpha=0.5)
            else:
                ax.scatter(x[indices2], y[indices2], z[indices2], c='k', s=2, alpha=0.5)
            # Plot contour box
            plotting.plot_contour_box(ax, x, y, z, cv[timestep,:])
        else:
            plt.figure(figsize=(9, 2))
            plt.scatter(x[indices2], y[indices2], c='k', s=2, alpha=0.5)
            plt.xlim([-25, 65])
            plt.ylim([-10, 10])
            plt.axis('equal')
        plt.savefig(os.path.join(PLTDIR, f'{fileprefix}_frame_{ts:04d}_{args.subsample}.png'), dpi=100, bbox_inches='tight')

        # spatial plot with samples colored according to cluster values
        plt.clf()
        if args.dims == 3:
            fig = plt.figure(figsize=(10, 8))
            ax = plt.subplot(111, projection='3d')
            ...
            if args.dtype == 'npz' or args.dtype == 'sst-binary':
                x_indices, y_indices, z_indices = np.unravel_index(indices2, (x.shape[0], y.shape[0], z.shape[0]))
                ax.scatter(x[x_indices], z[z_indices], y[y_indices], c=kmeans.labels_[indices2], s=2, \
                           cmap='tab10', vmin=-0.5, vmax=max(kmeans.labels_) + 0.5, alpha=0.5)
            else:
                ax.scatter(x[indices2], y[indices2], z[indices2], c=kmeans.labels_[indices2], s=2, \
                           cmap='tab10', vmin=-0.5, vmax=max(kmeans.labels_) + 0.5, alpha=0.5)
            # Plot contour box
            plotting.plot_contour_box(ax, x, y, z, cv[timestep,:])
        else:
            plt.figure(figsize=(9, 2))
            plt.scatter(x[indices2], y[indices2], c=kmeans.labels_[indices2], s=2, \
                        cmap='tab10', vmin=-0.5, vmax=max(kmeans.labels_) + 0.5, alpha=0.5)
            plt.xlim([-25, 65])
            plt.ylim([-10, 10])
            plt.axis('equal')
        plt.savefig(os.path.join(PLTDIR, f'{fileprefix}_frame_{ts:04d}_{args.subsample}_cluster.png'), dpi=100, bbox_inches='tight')
        ...
        plt.yscale('log')
        plt.legend()
        plt.savefig(os.path.join(PLTDIR, f'{fileprefix}_prob_dist_subsampled_{ts:04d}.png'), dpi=100)

    return subsampled_X, subsampled_Y, indices2
...
if rank == 0:
    ...
    dfpath = os.path.join(SNPDIR, f"{fileprefix}_{DRAWFN}")
    ...
    if args.snapshot and os.path.exists(dfpath):
        ...
        print(f'Shape of data: NN Input/observation - {X.shape}, NN Output/targets - {Y.shape}, Timesteps - {args.num_timesteps}')
    
        if args.saveData: np.savez(dfpath, X=X, Y=Y, cv=cv, x=x, y=y, z=z)
...
# =================================
# Set other parameters for sampling
...
timestep_tasks = np.arange(0, num_timesteps - args.window, args.window)
ntasks = len(timestep_tasks)

# Define how the loop work will be split across processes
tasks_per_process = ntasks // size  # Number of tasks per process
extra_tasks = ntasks % size  # Extra tasks for some processes if n is not perfectly divisible
if rank==0: print(f"Total num tasks: {ntasks}\nnum task per rank = {tasks_per_process}\nextra tasks = {extra_tasks}")

# Determine which part of the loop this process will handle
start = rank * tasks_per_process + min(rank, extra_tasks)
end = start + tasks_per_process + (1 if rank < extra_tasks else 0)

# start mpi processes
Xout_local = []
Yout_local = []
rank_ids_local = []  # to check data order
for i in range(start, end):
    timestep = timestep_tasks[i]
    subsampled_X, subsampled_Y, indices2 = time_step(timestep)
    Xout_local += [subsampled_X,]
    try:
        Yout_local += [subsampled_Y,]
    except Exception as e:
        raise Exception("Try removing ./snapshots/raw_data.npz and re-running" + str())
    rank_ids_local += [i,]

# =================================
# Gather the results from all processes at rank 0
Xout_full = comm.gather(Xout_local, root=0)
Yout_full = comm.gather(Yout_local, root=0)
rank_ids = comm.gather(rank_ids_local, root=0)
# check if the gathered data is in right order
if rank == 0:
    rank_ids = [item for sublist in rank_ids for item in sublist]  # Flatten the list
    print(f"Rank IDs: {rank_ids}")

if rank == 0:
    Xout_full = [item for sublist in Xout_full for item in sublist]  # Flatten the list
    Yout_full = [item for sublist in Yout_full for item in sublist]  # Flatten the list
    
    # save list elements to np array
    for i in range(len(Xout_full)):
        Xout[i,:] = Xout_full[i]
        Yout[i,:] = Yout_full[i]
    ...
    outfile = os.path.join(SNPDIR, outfilename)
    
    if args.dtype == 'npz' or args.dtype == 'sst-binary': # TODO: Not sure why we are saving indices2
        x_indices, y_indices, z_indices = np.unravel_index(indices2, (x.shape[0], y.shape[0], z.shape[0]))
    ...
    if args.saveData: np.savez(outfile, **arrays)
